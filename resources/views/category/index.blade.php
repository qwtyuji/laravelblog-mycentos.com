            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <div class="col-md-2 p-h-xs">
                        <a href="{{url('dashboard/category/create')}}"><button class="btn btn-info btn-block">添加</button></a>
                    </div>
                    ...
                    <table class="footable table table-stripped toggle-arrow-tiny table-bordered"
                        ...
                        <tr>
                            <td>{{$vo->name}}</td>
                            <td>{{$cateList[$vo->pid]}}</td>
                            <td>{{config('hupo.status')[$vo->status]}}</td>
                            <td>{{$vo->updated_at}}</td>
                            <td>{{$vo->created_at}}</td>
                            <td>
                                <a href="{{url('dashboard/category',$vo->id)}}" class="btn btn-xs btn-primary">查看</a>
                                <a href="{{url('dashboard/category',[$vo->id,'edit'])}}" class="btn btn-xs btn-success">修改</a>
                                <form action="{{url('dashboard/category',$vo->id)}}" method="post" class="form-horizontal" style="display: inline-block">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
